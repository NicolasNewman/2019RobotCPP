...
WheelModule::WheelModule(int anglePort, int speedPort, char id[2]) : Subsystem("WheelModule") {
...
double WheelModule::adjustAngle(double angle) {
  double target = 0;
  double current = round(angleMotor.GetSelectedSensorPosition(0) / COUNTPERDEG);
  double Rcurrent = 0;
